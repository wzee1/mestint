...
def Hill_Climbing_for_3Cup(problem):
    state = Node(problem.initial)
    while True:
        if problem.goal_test(state.state):
            return state
        ...
        for s_test in succesors:
            if heuristic_for_3Cup(state.state)>=heuristic_for_3Cup(s_test.state):
        ...
        for i in range(1, len(test_succesors)):
            h=heuristic_for_3Cup(test_succesors[i].state)
            m=heuristic_for_3Cup(test_succesors[0].state)
            if h < m:
                best_succesors=[]
